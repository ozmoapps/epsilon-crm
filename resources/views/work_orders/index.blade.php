<x-app-layout>
    <x-slot name="header">
        <x-partials.page-header title="{{ __('İş Emirleri') }}" subtitle="{{ __('İş emri süreçlerini takip edin.') }}">
            <x-slot name="actions">
                ...
                <x-ui.dropdown align="right" width="w-64">
                    ...
                    <x-slot name="content">
                        ...
                        @forelse($savedViews as $view)
                            <a href="{{ route('work-orders.index', $view->query) }}" class="block px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out flex justify-between items-center">
                                <span>{{ $view->name }}</span>
                                @if($view->is_shared)
                        ...
                        <a href="{{ route('saved-views.index', ['scope' => 'work_orders']) }}" class="block px-4 py-2 text-sm leading-5 text-indigo-600 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out text-center font-medium">
            ...
            <x-slot name="filters">
                <form method="GET" action="{{ route('work-orders.index') }}" class="contents">
                    ...
                    <div class="col-span-1 flex items-end justify-end space-x-2">
                        ...
                        <x-button href="{{ route('work-orders.index') }}" variant="secondary" class="w-full sm:w-auto justify-center">{{ __('Temizle') }}</x-button>
                    </div>
                </form>
            </x-slot>
        </x-partials.filter-card>
                            ...
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center space-y-3">
                                    ...
                                    <div class="text-center">
                                        ...
                                        <p class="mt-1 text-sm text-gray-500">{{ __('Arama kriterlerinize uygun iş emri bulunmuyor.') }}</p>
                                        <div class="mt-4">
                                            <x-button href="{{ route('work-orders.create') }}" size="sm">{{ __('Yeni İş Emri Oluştur') }}</x-button>
                                        </div>
                                    </div>
                                </div>
                            </td>
