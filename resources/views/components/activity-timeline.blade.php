...
@php
    ...
    $actionLabels = [
        ...
        'sales_order_deleted' => 'Satış siparişi silindi',
    ...
    $actionIcons = [
        ...
        'delete_blocked' => 'x',
        'sales_order_deleted' => 'trash',
        'contract_deleted' => 'trash',
    ];
    $statusLabelsByType = [
...
@endphp
    ...
    <div class="space-y-0" x-data="{ expanded: false }">
        @php
            $limitedLogs = $logs->take($limit);
            $remainingLogs = $logs->skip($limit);
        ...
        @if($hasMore)
            <div class="pt-3 text-center border-t border-slate-50 mt-2">
                <button
                    type="button"
                    class="text-xs font-semibold text-brand-600 hover:text-brand-700 hover:underline"
                    x-on:click="expanded = !expanded"
                    x-text="expanded ? '{{ __('Daha az göster') }}' : '{{ __('Daha fazlasını göster') }}'"
                ></button>
            </div>
        @endif
    </div>
