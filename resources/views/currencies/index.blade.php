<x-app-layout>
    <x-slot name="header">
        <x-page-header title="{{ __('Para Birimleri') }}" subtitle="{{ __('Kullanılan para birimlerini yönetin.') }}">
            <x-slot name="actions">
                <x-ui.button href="{{ route('currencies.create') }}" size="sm">
                    {{ __('Yeni Para Birimi') }}
                </x-ui.button>
            </x-slot>
        </x-page-header>
    ...
    <div class="space-y-6">
        <x-ui.card>
            ...
            <form method="GET" action="{{ route('currencies.index') }}" class="flex flex-col gap-4 sm:flex-row sm:items-end">
                <div class="flex-1">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <x-ui.button href="{{ route('currencies.index') }}" variant="secondary" size="sm">{{ __('Temizle') }}</x-ui.button>
                </div>
            </form>
        </x-ui.card>
                        ...
                        <tr class="odd:bg-white even:bg-slate-50 hover:bg-slate-100/60">
                            ...
                            <td class="px-4 py-3 text-right">
                                <x-ui.dropdown align="right" width="w-44">
                                    ...
                                    <x-slot name="content">
                                        <a href="{{ route('currencies.show', $currency) }}" class="{{ $actionItemClass }}">
                                            <x-icon.info class="h-4 w-4 text-sky-600" />
                                            {{ __('Görüntüle') }}
                                        </a>
                                        <a href="{{ route('currencies.edit', $currency) }}" class="{{ $actionItemClass }}">
                                            <x-icon.pencil class="h-4 w-4 text-indigo-600" />
                                            {{ __('Düzenle') }}
                                        </a>
                                        <form id="currency-delete-{{ $currency->id }}" method="POST" action="{{ route('currencies.destroy', $currency) }}" class="hidden">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                        <x-ui.confirm
