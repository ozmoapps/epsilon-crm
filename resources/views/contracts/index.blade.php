<x-app-layout>
    ...
    <div class="mt-6 space-y-4">
        ...
        @forelse ($contracts as $contract)
            <x-card>
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <p class="text-xs text-gray-500">{{ $contract->contract_no }}</p>
                        <p class="text-base font-semibold text-gray-900">{{ $contract->customer_name }}</p>
                        <p class="text-xs text-gray-500">{{ $contract->issued_at?->format('d.m.Y') }}</p>
                    </div>
        ...
        @empty
            <x-card>
                <p class="text-sm text-gray-500">{{ __('Sözleşme bulunamadı.') }}</p>
            </x-card>
        @endforelse
