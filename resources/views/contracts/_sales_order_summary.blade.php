...
<x-card>
    ...
    <div class="grid gap-4 text-sm sm:grid-cols-2">
        <div>
            <p class="text-xs tracking-wide text-gray-500">{{ __('Sipariş No') }}</p>
            <p class="text-base font-medium text-gray-900">{{ $salesOrder->order_no }}</p>
        </div>
        <div>
        ...
        <div>
            ...
            <p class="text-base font-medium text-gray-900">{{ $formatMoney($salesOrder->subtotal) }} {{ $currencySymbol }}</p>
        </div>
        <div>
            <p class="text-xs tracking-wide text-gray-500">{{ __('Vergi Toplamı') }}</p>
            <p class="text-base font-medium text-gray-900">{{ $formatMoney($salesOrder->vat_total) }} {{ $currencySymbol }}</p>
        </div>
        <div>
            <p class="text-xs tracking-wide text-gray-500">{{ __('Genel Toplam') }}</p>
            <p class="text-base font-medium text-gray-900">{{ $formatMoney($salesOrder->grand_total) }} {{ $currencySymbol }}</p>
        </div>
    </div>
</x-card>
